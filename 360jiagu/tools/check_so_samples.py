...
check_so_samples.py — 双向回归断言（专治"只拿自己造的样本验证自己做的检测器"的循环论证）
  ...
  · 防回归：libtarget_360.so / libtarget_360_variant.so 仍须判为加壳
            （确认 B13 改造没有破坏原有的 UPX/360 判定）
...
def _run(name):
    ...
    with contextlib.redirect_stdout(io.StringIO()):
        return detect_360.detect(path, brief=True)
...
def main():
    ...
    # 防回归：360 / 变种 360 仍须判为加壳，且不得被 B13 改动破坏
    for name in ("libtarget_360.so", "libtarget_360_variant.so"):
        r = _run(name)
        if r is None:
