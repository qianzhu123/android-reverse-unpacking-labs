#!/usr/bin/env python
# detect_360.py — 基于"结构特征"判断加壳 / 是否 360 加固 / 是否变种
# 不使用文件名、不依赖 UPX 字符串，全部靠 ELF 结构与统计特征判定。
#
# 模型（详见 README）：360 加固的 native 层本质是"改版 UPX"。
#   - 结构特征（节区表剥离 / 高熵 / 大解压缓冲 / 入口在 stub / 内存文件体积比 / 尾部 sz_unc）
#     与标准 UPX 完全一致 —— 这是判"是否加壳"的依据。
#   - "是否 360"由两路确认：
#       ① 变种 360 把 UPX! 改写成 360 的 4 字节 token（默认 JG!!）；
#       ② 360 的 payload / stub 可能留下明文标记（"360jiagu" / "360 4.24"）。
#     这两路只是"命名/归属"依据；真正区分"加壳 vs 未加壳"的仍是上面的结构特征。
#
# 用法:
#   python tools/detect_360.py <file>            # 详细判定（含教学解释）
#   python tools/detect_360.py <file> --brief    # 只看结论
#   python tools/detect_360.py <file> --verify   # 实证确认被篡改的魔数（需 upx）
#   python tools/detect_360.py <file> --verify --upx <upx 路径>
#
# 【可复用】不依赖任何具体项目：upx 通过 PATH / 环境变量 UPX / --upx 定位。
import sys, os, struct, math, collections, subprocess, tempfile
...
# 360 明文标记（payload 或 stub 中可能留下）
JIAGU_MARKERS = (b"360jiagu", b"360 4.24", b"Jiagu")
# 变种 360 改写后的魔数占位（默认由 make_360_variant.py 写入）
JG_TOKEN = b"JG!!"

# 认知边界：明确告诉使用者本工具【不】覆盖哪些方案，
# 避免把"没看到已知特征"当成"没有加固"（这是最危险的误判方向）。
COVERAGE_NOTICE = (
    "本工具覆盖 UPX/360 类壳（魔数/结构特征）与 SO 加壳/自实现 Linker 的 ELF 结构异常"
    "（节头剥离 B13）。\n"
...
def _upx_from_path():
    """在 PATH 里找 upx（跳过当前目录，避免项目自带 upx.exe 意外盖掉系统 upx）。"""
...
def find_upx(explicit=None):
    """定位 UPX：--upx > 环境变量 UPX > 项目根 upx.exe/upx > PATH > tools/upx396.exe。
...
def magic_candidates(d, tail=96, lo=2, hi=16):
    """被篡改的 360/UPX 魔数候选（与 detect_packer 同逻辑）。"""
    ...
    for g, c in cnt.items():
        if not (lo <= c <= hi):
            continue
        if g[0] == g[1] == g[2] == g[3] and g[0] in (0x00, 0xFF):
            continue
        if g not in tailb:
            continue
        offs = []
...
def detect(path, brief=False, verify=False, upx=None):
    ...
    feats.append(("F3 大解压缓冲段", desc,
        "360/UPX 的 UPX0：文件里几乎为空、内存中却很大(解压目标区)；要求 memsz>=32KB 排除普通 .bss", f3, 3))
    ...
    feats.append(("F6 'UPX!' 魔数", "出现 %d 次" % upx_bang,
        "标准 UPX / 可恢复 360 的特征；变种会把它改写掉", f6, 3))
    ...
    feats.append(("F8 疑似被篡改魔数",
        ...
        "无 UPX!，但有'重复(2~16次)且出现在尾部'的 4 字节 token => 魔数被改写过（360 的 JG!! 等）", f8, 3))
    ...
    # 360 归属指示（不参与打分，仅辅助命名）
    print("-" * 70)
    print("360 归属指示（非打分，仅用于'是否 360'命名）:")
    print("  明文 360 标记(360jiagu/360 4.24/Jiagu): %s" % ("存在" if jiagu_marker else "无（脱壳后可见 payload 标记）"))
    print("  变种 360 token(%r): 出现 %d 次" % (JG_TOKEN, jg_token_hits))
    ...
    packed = False
    if f6 and score >= 6:
        if jiagu_marker or jg_token_hits > 0:
            verdict = "标准 360 加固（改版 UPX，UPX! 保留，可自动解包）"
        else:
            verdict = "标准 UPX 壳（UPX! 保留，可自动解包）— 未检出 360 标记"
        action = "直接 upx -t 验证，然后 upx -d 解包，不必手脱"
        packed = True
    elif f8 and score >= 6:
        if jg_token_hits > 0 or jiagu_marker:
            verdict = "★ 变种 360 加固（UPX! 被 360 改写，结构仍在）"
        else:
            verdict = "★ 变种 UPX/未知壳（UPX! 被抹，结构仍在）"
        action = "走解法 A：把候选 token 还原成 'UPX!'（必须全部还原）后 upx -d；失败则走解法 B 内存 dump"
        packed = True
    elif score >= 4:
        verdict = "疑似加壳（非 360 或深度魔改）"
    ...
    if f8 and cands:
        ...
        print("   注意: 必须把所有出现位置都改回，UPX/360 校验和覆盖全部内嵌魔数，只改一处仍会失败。")
...
if __name__ == "__main__":
    ...
    if not args:
        print("用法: python tools/detect_360.py <file> [--brief] [--verify] [--upx <path>]")
        sys.exit(1)
    for p in args:
        detect(p, brief, verify, upx)
