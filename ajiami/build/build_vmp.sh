...
#   samples/app_vmp.apk         可直接被 detect.py 判定的 VMP 样本
#
# 教学目的（见 README §2.5 / §6）：
...
APK_SIGNER sign --ks "$KEYSTORE" --ks-key-alias "$KEY_ALIAS" --ks-pass pass:"$KEY_STOREPASS" --key-pass pass:"$KEY_KEYPASS" --out samples/app_vmp.apk build/vmp/vmp_aligned.apk

echo
echo "[+] samples/app_vmp.apk   DEX VMP 教学样本"
echo "[+] 下一步：python tools/detect.py samples/app_vmp.apk   （观察 B3 是否失效）"
