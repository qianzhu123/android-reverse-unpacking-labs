...
>    python tools/unpack_v1.py samples/app_packed_v1.apk analysis_output/out.dex --pristine samples/classes_orig.dex
...
> ⚠️ **零绝对路径约定**（硬约束）
> 工程里**任何文件**（脚本 / 构建配置 / 源码 / README / 生成产物）都不得出现
...
1. 文档（最重要）
   - 全部知识点只写进项目根目录 README.md 一个文件，不要再建 docs/ 或其他 md 文档。
   - 固定章节骨架（照抄，不要自创编号）：
       §0 简介：0.1 一句话定位 / 0.2 为什么自造样本（建模说明）/ 0.3 样本清单表 /
               0.4 三分钟跑一遍（最小闭环命令 4~8 行）
       §1 知识点：每节固定五件套 —— 是什么 / 怎么看（结构特征+阈值）/ 验证脚本 /
                  实测输出 / 失效边界；有样本标 [可验证]，纯理论标 [知识框架]
       §2 判定：特征对照表 + 判定脚本实跑输出 + 阈值设计踩过的坑
       §3 解法 A · 脚本路线（先跑通，先给正确答案）
       §4 解法 B · 手动路线（字节级；每步标注「对应 §3 脚本的第几行输出」；末尾带速查表）
       §5 解法 C · 兜底路线（内存 dump / 动态；没有就删掉这一章）
       §6 决策流程（决策树）
       §7 验证判据（体积 / 锚点 / sha256 / 差异字节 + 首个差异偏移）
       §8 踩坑（工具链 / 环境 / 阈值 / 误判）
       §9 反复练习：pristine/ + reset_lab.py（status / restore / backup）
       §10 命令速查 / §11 文件索引 / §12 一句话记住
...
8. 零绝对路径（硬性）
   - 源码、构建脚本、Python 工具、README、生成产物（*.json / *.txt 报告）中
     一律不得出现盘符路径（`D:\`、`C:\`）或用户目录（`/c/Users/`、`/d/`）。
   - 文档里提到工程位置统一写「工程根目录（本 README 所在目录）」，
     示例命令用相对路径。
   - **唯一例外**：README 的 `cd` 示例可以写本机实际路径（便于复制执行），
     但脚本 / 构建配置 / 生成产物里绝对不能有。
   - 交付前自查：全库搜索 `:\` 与 `/Users/`，除 README 的 `cd` 示例与历史产物外应为 0 命中；
...
9. 判定工具的验证纪律（硬性，专治循环论证）
   ...
   - README §8（踩坑 / 失效边界）必须记录：哪条判据在什么真实场景下会误报、根因是什么、怎么改的。
...
| 零绝对路径 | 全库禁止盘符/用户目录；工程根由脚本自身位置推导；产物输出相对路径；**唯一例外**：README 的 `cd` 示例可写本机实际路径 |
