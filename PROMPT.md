...
> ⚠️ **文档写作约定**
> 1. **先脚本后手动**：章节顺序 = 原理 → 脚本/工具跑通 → 手动复现。先用自动化给出正确答案，
>    再拆解为什么；不要一上来就铺手动字节级步骤。
...
>    先提交 `README.md`（仓库总览）/ `LICENSE` / `.gitignore` / `PROMPT.md`，再分别提交每个工程（每个工程含 `SCRIPT.md` + `MANUAL.md`）；
...
1. 文档（最重要）
   - 全部知识点只写进两份平行文档：脚本路线 `SCRIPT.md` + 手动路线 `MANUAL.md`（同一 §0–§8 骨架），不要再建 docs/ 或其他 md 文档；项目根 `README.md` 不再需要。
   - 固定章节骨架（照抄，不要自创编号）：两份文档共用同一套 §0–§8 骨架，区别只在「手法」——
       SCRIPT.md 走脚本路线（detect / unpack / verify 等工具实跑），
       MANUAL.md 走手动路线（unzip / aapt2 / xxd / 熵 等通用工具逐字节复现）。
       §0 简介：0.1 一句话定位 / 0.2 为什么自造样本（建模说明）/ 0.3 样本清单表 /
               0.4 三分钟跑一遍（最小闭环命令 4~8 行）
       §1 工程结构 / 样本清单（三栏：apks / dex / payloads）
       §2 知识点：每节固定五件套 —— 是什么 / 怎么看（结构特征+阈值）/ 验证脚本 /
                  实测输出 / 失效边界；有样本标 [可验证]，纯理论标 [知识框架]
       §3 判定：特征对照表 + 判定脚本实跑输出 + 阈值设计踩过的坑
       §4 脱壳：脚本路线用工具 / 手动路线用通用工具，逐字节对照
       §5 验证判据（体积 / 锚点 / sha256 / 差异字节 + 首个差异偏移）
       §6 决策流程（决策树）
       §7 反复练习：pristine/ + reset_lab.py（status / restore / backup）
       §8 踩坑（工具链 / 环境 / 阈值 / 误判 / 失效边界）
...
4. 脚本与手动（先脚本后手动）
   - 必须给出可手工复现的步骤：定位到具体字节偏移、能用十六进制编辑器自己改、
     每条命令的作用都要说明。
   - 脚本只能是这些步骤的自动化，不能是唯一答案。
...
9. 判定工具的验证纪律（硬性，专治循环论证）
   ...
   - SCRIPT.md / MANUAL.md §8（踩坑 / 失效边界）必须记录：哪条判据在什么真实场景下会误报、根因是什么、怎么改的。
...
| 文档 | 两份平行文档 `SCRIPT.md`(脚本)+`MANUAL.md`(手动)，同一 §0–§8 骨架 |
...
| 文档顺序 | 原理 → 脚本跑通 → 手动复现；手动步骤标注对应脚本输出 |
...
| 存量工程迁移 | 早期交付的单 README 工程（如 360jiagu / upx_practice）：内容达标的可暂按原状发布，但下次大改时必须顺势拆成 SCRIPT.md + MANUAL.md 双文档并对齐 §0–§8 骨架；新工程一律直接用双文档，禁止再交付单 README |
